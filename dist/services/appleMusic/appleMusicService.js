 ...
 */
const findBestMatch = async (releaseForSearch, settingsForThisRun, parentSignal, releaseForScoring) => {
    ...
    for (const strategy of strategies) {
        ...
        while (hasMorePages) {
            if (parentSignal?.aborted)
                throw new DOMException('Aborted by parent', 'AbortError');
            try {
                const data = await fetchFromAppleMusic(strategy.query, strategy.entity, !!strategy.omitEntity, strategy.attribute, currentOffset, parentSignal);
                if (totalResultsFromServer === -1)
                    totalResultsFromServer = data.resultCount;
                if (data.resultCount > 0 && data.results.length > 0) {
                    ...
                    const typeFilteredResults = resultsToProcess.filter(result => {
                        ...
                        if (discogsType !== ReleaseType.SINGLE && appleType === ReleaseType.SINGLE)
                            return false;
                        if (discogsType !== ReleaseType.EP && appleType === ReleaseType.EP)
                            return false;
                        if (discogsType === ReleaseType.SINGLE && appleType === ReleaseType.ALBUM)
                            return false;
                        return true;
                    });
                    const validatedResults = typeFilteredResults.filter(result => {
                        if (strategy.attribute === 'albumTerm')
                            return calculateFuzzyScore(strategy.query, result.collectionName) >= ANCHOR_FIELD_VALIDATION_THRESHOLD;
                        if (strategy.attribute === 'artistTerm')
                            return calculateFuzzyScore(strategy.query, result.artistName) >= ANCHOR_FIELD_VALIDATION_THRESHOLD;
                        return true;
                    });
                    const preFilteredResults = validatedResults.filter(result => {
                        ...
                        if (strategy.type === AppleSearchStrategyType.ALBUM_PLUS_YEAR)
                            return artistScore > PRE_FILTER_THRESHOLD;
                        if (strategy.type === AppleSearchStrategyType.ARTIST_PLUS_YEAR)
                            return albumScore > PRE_FILTER_THRESHOLD;
                        if (strategy.type === AppleSearchStrategyType.ARTIST_ONLY)
                            return artistScore > PRE_FILTER_THRESHOLD;
                        if (!strategy.attribute)
                            return Math.max(artistScore, albumScore) > PRE_FILTER_THRESHOLD;
                        return true;
                    });
                ...
                if (overallBestScore >= ACCEPTANCE_THRESHOLD) {
                    ...
                    if (data.results.length < 200 || currentOffset >= totalResultsFromServer)
                        hasMorePages = false;
                }
            }
            catch (e) {
        ...
        if (overallBestScore >= ACCEPTANCE_THRESHOLD)
            break; // Move to the next strategy if no good match found
    }
    return { bestMatch: overallBestMatch, bestScore: overallBestScore, bestStrategy: bestMatchStrategy };
};
const processFinalResult = (result, release) => {
 ...
 */
const handleCollaborationFallback = async (initialBestResult, release, settings, parentSignal) => {
    ...
    const artistCorrectionSettings = { ...settings, artistSource: 'apple', albumSource: 'discogs' };
    ...
    for (let i = 0; i < originalArtists.length; i++) {
        ...
        if (hasFoundCorrection) {
            ...
            const scoringReleaseWithSubstitution = {
                ...release,
 ...
 */
export const fetchAppleMusicMetadata = async (release, settings, parentSignal) => {
