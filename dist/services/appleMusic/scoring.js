...
export function getDiscogsReleaseType(release) {
    ...
    if (descriptions.includes('single'))
        return ReleaseType.SINGLE;
    if (descriptions.includes('ep'))
        return ReleaseType.EP;
    if (descriptions.includes('compilation'))
        return ReleaseType.COMPILATION;
    if (descriptions.includes('album') || descriptions.includes('lp'))
        return ReleaseType.ALBUM;
    return ReleaseType.UNKNOWN;
}
export function getAppleReleaseType(result) {
...
export function getScores(discogs, apple) {
 ...
 */
export function extractLabelFromCopyright(copyright) {
    if (!copyright)
        return '';
    let label = copyright;
 ...
 */
export function calculateTruthScore(discogs, apple, strategy, settings) {
 ...
 */
export function isBetterTieBreak(discogs, newResult, currentBestResult, settings) {
    const newScores = getScores(discogs, newResult);
    const currentScores = getScores(discogs, currentBestResult);
    let secondaryScoreNew;
    let secondaryScoreCurrent;
    ...
    if (secondaryScoreNew > secondaryScoreCurrent)
        return true;
    if (secondaryScoreNew < secondaryScoreCurrent)
        return false;
