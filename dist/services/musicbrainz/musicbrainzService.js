...
const calculateMBScore = (discogs, mbRelease) => {
    // 1. Barcode match (if available) - Very strong signal
    if (mbRelease.barcode && discogs.identifiers) {
        const hasBarcodeMatch = discogs.identifiers.some(id => id.type === 'Barcode' && id.value.replace(/\s+/g, '') === mbRelease.barcode);
        if (hasBarcodeMatch)
            return 1.0;
    }
...
export const fetchMusicBrainzMetadata = async (release, signal) => {
