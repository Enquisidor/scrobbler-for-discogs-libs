 ...
 */
export function areReleasesEqual(a, b) {
    ...
    if (infoA.year !== infoB.year)
        return false;
    if (infoA.artist_display_name !== infoB.artist_display_name)
        return false;
    if (infoA.cover_image !== infoB.cover_image)
        return false;
    // Check Formats (length and basic properties)
    if ((infoA.formats?.length || 0) !== (infoB.formats?.length || 0))
        return false;
    if (JSON.stringify(infoA.formats) !== JSON.stringify(infoB.formats))
        return false;
    return true;
}
/**
 ...
 */
export function mergePageIntoCollection(currentCollection, incomingPage) {
